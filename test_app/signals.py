...
from django_core_api import signals
...
def notify_minister_of_magic(sender, instance, created, **kwargs):
    tasks.NotifyMinisterOfMagicTask().apply()
...
def memory_has_been_erased(sender, instance, **kwargs):
    tasks.NotifyMinisterOfMagicTask().delay(recovered=False)
...
def memory_has_been_recovered(sender, instance, **kwargs):
    tasks.NotifyMinisterOfMagicTask().delay(recovered=True)
